    ...
    <div class="flex flex-col rounded-lg w-full gap-3 h-[80vh]">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-5 snap-x md:py-8 md:px-8 overflow-y-auto">
            {#each $userPostsStore as post} 
                <ElonProfile status={post.status} price={post.price} id={post.id} author_name={post.user.name} user_role={post.user_role} from_loc={post.from_location} to_loc={post.to_location} go_time={post.go_time} count={post.count} addition={post.addition}/>
            {/each}
        </div>
    </div>
